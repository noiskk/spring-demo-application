...
public class RentalDBTest {
    ...
    void testFindRental() {
        ...
        System.out.println("member_no = " + rental.getMember().getMemberNo());
        System.out.println("member_name = " + rental.getMember().getMemberName());
    ...
    void testSaveRental() {
        ...
        Member member = em.find(Member.class, "M004");
        Rental rental = new Rental("R004", "B004", member,
                LocalDate.now(), null, false);
        em.persist(rental);

        tx.rollback(); // DB에 안 남기고 테스트만

        // rollback 후 조회는 안 되므로 persist 성공 자체가 통과
    }
